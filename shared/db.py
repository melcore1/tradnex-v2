...
def get_connection() -> sqlite3.Connection:
    ...
    # check_same_thread=False is needed for FastAPI: sync deps run in a thread
    # pool while async path operations run in the event-loop thread, so a
    # connection acquired in the dep can be used in either. Safe because we
    # use WAL mode + a fresh connection per request (no shared state).
